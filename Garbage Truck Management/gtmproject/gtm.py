...
def login(select_role):
    ...
    if select_role == "ADMIN":
        ...
        f.close()
        if userv:
            messagebox.showinfo("User Info",'I, a responsible citizen of India solemnly pledge to'
                                'take concern over the environment,make sure to separate degradable' 
                                'and non degradable waste,and dispose it properly. I will try my best'
                                'to reduce waste and reuse things when I can and recycle them properly.'
                                'I will help to keep my surroundings clean. I will also encourage others'
                                'to be mindful of waste and take care of our environment')
...
def openlogin():
    ...
    if select_role in ["USER", "ADMIN"]:
        ...
        errorlbl.config(text="Please select a role beforeproceeding.")
def reg_func():
    root.destroy()
    import register
pht = Image.open("gtmproject//assets//b2.jpg")
...
imglabel.create_image(0, 0, anchor='nw', image=bgimg)
imglabel.create_text(310, 100, text="R3-Waste Management",font=("Comic Sans MS", 14, "bold"), fill="black")
imglabel.pack()
#Roleselection
roles=["USER", "ADMIN"]
